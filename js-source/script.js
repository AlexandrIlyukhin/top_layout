...
const getData = () => {
    data.month = startCalendar.month; //data.students = startStudent;
    data.colorDays = startCalendar.dayClicks
};
...
$('.table-calendar .daymonth').on('click', (event) => {
    const day = $(event.target).text();
    const calendarId = $(event.target).parents('.table-calendar').attr('data-month-id');
...
$('.table-calendar .daymonth').on('contextmenu', (event) => {
    ...
    startCalendar.dayClicks.splice(dayClick, 1); //TODO разобратьс с удалением и окрашиванием дня
    //$(event.target).css('background', getActualColor());
    //$(event.target).css('background', dayClick.color);
});
// TODO Обсуждение как нужно удалять цвет и убирать его из базы. 1.03
